...
public class QuizPrototype : MonoBehaviour {
    ...
    [SerializeField] GameObject ResultControl;
   ...
   //         }
        ...
        if (questionList[questionIndex] == "Quiz57")
        {
            ...
            Questionlabel.text = "プリントコンタクト部1.2の各項目の許容範囲は？\n打痕:(X)mm未満\nキズ:幅(Y)mm未満\nこぶ状の突起S:(Z)mm未満\n";

            AnswerA.text = "(X) 1:0.2 2:0.5 (Y) 1:0.1 2:0.5 (Z) 1:0.2 2:0.2";
            array = new string[] { "(X) 1:0.2 2:0.5      (Y) 1:0.1 2:0.5\n (Z) 1:0.2 2:0.2", "(X) 1:0.2 2:0.5      (Y) 1:0.2 2:0.5\n (Z) 1:0.2 2:0.1", "(X) 1:0.2 2:0.2      (Y) 1:0.2 2:0.5\n (Z) 1:0.3 2:0.3", "(X) 1:0.2 2:0.2      (Y) 1:0.2 2:0.5\n (Z) 1:0.2 2:0.2" };
            ArrayButton();
        }
    ...
    void Question27() { AnswerA.text = "W:20% X:1個  Y:2個"; Questionlabel.text = "図に示す導体の幅が0.99mm～0.50mmの欠損部分の幅は仕上がり幅の(W)以下、個数(1導体(X)個、100×100m中に(Y)個以下）の許容差は？"; array = new string[]{ "W:20% X:1個  Y:2個", "W:25%  X:2個、Y:2個","W:30%。X:1個、Y:2個","W:20%以下  X:2個  Y:2個"}; }
    void Question28() { AnswerA.text = "w:仕上がり幅の25%以下 ℓ:仕上がり導体幅以下  個数:X:3y3"; Questionlabel.text = "図に示す導体の幅が0.49～0.20mmの欠損部分の幅(w)、長さ(l)・個数(1導体(X)個、100×100m中に(Y)個以下）の許容差は？"; array = new string[] {Quiz28, "w:仕上がり幅の30%以下 ℓ:仕上がり導体幅以下  個数:X:3y3", "w:仕上がり幅の20%以下 ℓ:仕上がり導体幅以下  個数:X:3y3", "w:仕上がり幅の30%以下 ℓ:仕上がり導体幅以下  個数:(X)=1 (Y)=2" }; }
    void Question29() { AnswerA.text = "w:仕上がり幅の20%以下 ℓ:仕上がり導体幅以下  個数:X=1 Y=2"; Questionlabel.text = "図に示す導体の幅が0.19mm以下の欠損部分の幅(w)、長さ(l))・個数(1導体(X)個、100×100m中に(Y)個以下）の許容差は？"; array = new string[] {Quiz29, "w:仕上がり幅の25%以下 ℓ:仕上がり導体幅以下  個数: X=3 Y=3", "w:仕上がり幅の20%以下 ℓ:仕上がり導体幅以下  個数:X=3 Y=3", "w:仕上がり幅の30%以下 ℓ:仕上がり導体幅以下  個数:X=1 Y=2" }; }
    ...
    void Question31() { AnswerA.text = "X:0.2mm以下  Y:2個以下  Z:3個まで"; Questionlabel.text = "導体間隙部が1.0mm以上で図に示す導体残りがある時、導体の残りの幅(X)でその部分の設計導体間隙値を超えてはならない。個体は隣接する導体間に(Y)、かつ100×100mm中に(Z)"; array = new string[] { "X:0.2mm以下  Y:2個以下  Z:3個まで", "X:20%以下  Y:2個以下  Z:2個まで", "X:25%以下  Y:1個以下  Z:2個まで", "X:0.2mm以下。Y:1個以下、Z:2個まで" }; }
