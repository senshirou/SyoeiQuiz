...
public class QuizPrototype : MonoBehaviour {
   ...
   //         }
    ...
    void Question28() { AnswerA.text = "w:仕上がり幅の25%以下 ℓ:仕上がり導体幅以下  個数:X:3y3"; Questionlabel.text = "図に示す導体の幅が0.49～0.20mmの欠損部分の幅(w)、長さ(ℓ)・個数(1導体(X)個、100×100m中に(Y)個以下）の許容差は？"; array = new string[] {Quiz28, "w:仕上がり幅の30%以下 ℓ:仕上がり導体幅以下  個数:X:3y3", "w:仕上がり幅の20%以下 ℓ:仕上がり導体幅以下  個数:X:3y3", "w:仕上がり幅の30%以下 ℓ:仕上がり導体幅以下  個数:(X)=1 (Y)=2" }; }
    void Question29() { AnswerA.text = "w:仕上がり幅の20%以下 ℓ:仕上がり導体幅以下  個数:X=1 Y=2"; Questionlabel.text = "図に示す導体の幅が0.19mm以下の欠損部分の幅(w)、長さ(ℓ)・個数(1導体(X)個、100×100m中に(Y)個以下）の許容差は？"; array = new string[] {Quiz29, "w:仕上がり幅の25%以下 ℓ:仕上がり導体幅以下  個数: X=3 Y=3", "w:仕上がり幅の20%以下 ℓ:仕上がり導体幅以下  個数:X=3 Y=3", "w:仕上がり幅の30%以下 ℓ:仕上がり導体幅以下  個数:X=1 Y=2" }; }
