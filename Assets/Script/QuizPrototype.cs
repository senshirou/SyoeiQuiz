...
public class QuizPrototype : MonoBehaviour {
   ...
   //         }
    ...
    void Question27() { AnswerA.text = "W:20% X:1個  Y:2個"; Questionlabel.text = "図に示す導体の幅が0.99mm～0.50mmの欠損部分の幅(w)、長さ(ℓ)・個数(1導体(X)個、100×100m中に(Z)個以下）の許容差は？"; array = new string[]{ "W:20% X:1個  Y:2個", "W:25%  X:2個、Y:2個","W:30%。X:1個、Y:2個","W:20%以下  X:2個  Y:2個"}; }
